...
const remove_file = async (dir) => {
    let result = await eel.remove(dir)();
    if (result){
...
const renderWay = (pathParts) => pathParts.forEach((part, i) => {
    if (part != ""){
        ...
        let fp = pathParts.slice(0, i + 1).join("/");
        console.log(fp);
        span.addEventListener("click", () => openFolder(fp));
...
const removeBtn = document.querySelector("#rem_btn")
removeBtn.addEventListener("click", () =>{
    const checks = document.querySelectorAll(".check")
    const checked = Array.from(checks).filter(check => check.checked)
    const ways = checked.map(check => check.parentNode.parentNode.querySelector("td").textContent)
    const fullways = ways.map(way => fullPath + "/" + way.trim())
    console.log(fullways)
    for (let i; i < fullways.length; i++ ){
        remove_file(fullways[i])
    }
})
